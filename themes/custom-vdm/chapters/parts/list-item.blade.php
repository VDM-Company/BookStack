...
<a href="{{ $chapter->getUrl() }}" class="chapter entity-list-item @if($chapter->visible_pages->count() > 0) has-children @endif" data-entity-type="chapter" data-entity-id="{{$chapter->id}}">
    ...
    <div class="content">
        <h4 class="entity-list-item-name break-text"><span style="display: inline-block; vertical-align: middle;">@icon('chapter', ['style' => 'width: 24px; height: 24px;'])</span> {{ $chapter->name}}</h4>
